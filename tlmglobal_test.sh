...
WORKDIR=$HOME/scratch/`whoami`/test.`date +%s`
mkdir -p $WORKDIR

OUTPUTDIR=$HOME/scratch/`whoami`/test.`date +%s`/output
mkdir -p $OUTPUTDIR
BASEDIR=`pwd`
...
python3 tlm_sterodynamics_preprocess_thermalexpansion.py --scenario ssp585 --climate_data_file /opt/climate/offline_fair_cliamte.nc --pipeline_id tlm.global.ocean.tlm.sterodynamics
